import type { UsageData, ColorOverrides, ColorValue } from './types.js';
...
function resolveAnsi(value: ColorValue | undefined, fallback: string): string {
  ...
  return ANSI_BY_NAME[value as string] ?? fallback;
}

function withOverride(text: string, value: ColorValue | undefined, fallback: string): string {
  return `${resolveAnsi(value, fallback)}${text}${RESET}`;
}

function getQuotaColor(percent: number, colors?: Partial<ColorOverrides>): string {
...
function label(text: string, colors?: Partial<ColorOverrides>): string {
  return withOverride(text, colors?.label, DIM);
}

function critical(text: string, colors?: Partial<ColorOverrides>): string {
...
export function renderUsageLine(data: UsageData, barWidth: number = 10, colors?: Partial<ColorOverrides>): string {
...
export function renderLines(usages: UsageData[], error?: string, colors?: Partial<ColorOverrides>): string[] {
  if (error) {
  ...
  filtered.sort((a, b) => {
    if (a.windowLabel === 'Total') return 1;
    if (b.windowLabel === 'Total') return -1;
    return 0;
  });
